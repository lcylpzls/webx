...
- `unixShortDirCandidates()` 对 `SystemRoot` 判空：环境变量缺失时不再生成
  `"\\Temp"` 候选，避免极端环境（如 CI 容器）下产生无意义路径。
...
- README / docs 与当前代码同步：标准中间件形态、指标外置指南与决策记录更新；
- 清理历史 API 快照（api-v0.x / api-v1.0.0）与过期规划文档；
- 修正 v2 实验线历史标注。
...
> 注：以下 v2.x 条目对应 `/v2` 模块路径实验期的历史记录，
> 该实验未发布正式 tag，最终回退为无后缀路径；相关能力已并入
> v1.3.0 及后续版本，请以 v1.x 条目为准。
