<!-- v1.0.0 API 基线（冻结）；生成方式：go doc -all . / ./middleware / ./proxy / ./pprof -->
...
func BodyLimit(maxBytes int64) core.HandlerFunc
    BodyLimit 返回请求体大小限制中间件（默认文案）。 Content-Length 明确超限时直接返回 413；chunked 请求体由
    MaxBytesReader 兜底。

func BodyLimitWithOptions(maxBytes int64, opts BodyLimitOptions) core.HandlerFunc
    BodyLimitWithOptions 返回带文案选项的请求体大小限制中间件。 Content-Length 明确超限时直接返回
    413；chunked 请求体由 MaxBytesReader 兜底。
...
func WithDirector(fn func(*http.Request)) Option
    WithDirector 设置上游请求改写函数（如注入头、改写路径）， 叠加在默认地址改写之后（基于 Rewrite 实现）。
